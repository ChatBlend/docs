---
title: "Integrate  ChatBlend with Google Sheets"
description: "This page contains the stepwise tutorial for the integration of Google Sheets with  ChatBlend"
---

### **Why Use  ChatBlend with Google Sheets?**
...
The following conditions must be fulfilled to use Google Sheet Integration with  ChatBlend:

1. ChatBlend Account: The user must have an active  ChatBlend Account, and must have an active bot which they wish to connect.

<Info>
  Google Sheets integration is not available for MS Teams, Facebook, Instagram or Telegram.
</Info>

### **Step 1 of 4: Login to  ChatBlend Account and Select Application to Integrate**

1. Login to your  ChatBlend account. Now, select and click the chatbot you want to integrate with Google Sheets. Click on the “Third-party Apps Integration”.
...
2. Now, click allow for  ChatBlend to access your Google account.
3. After you click allow, the connection between  ChatBlend and Google Sheets is established, and you will be redirected back to  ChatBlend onto the following page.

### **Step 3 of 4: Set up Mapping Parameters**

1. Now, you need to map **parameters (fields) of Google Sheets** to the user response of the  ChatBlend question. In other words, you need to map the questions or custom attributes that will be saved to the Google Sheet. The fields that need to be entered are:
...
<Warning>
  Make sure you have a empty sheet made in your google sheet account, so it can be connect to  ChatBlend.
</Warning>

You can use the **\+Add Parameters** button to add more fields.

For example, you select a question: Please provide your name?. Then, you map it with the parameter -Name. As a result, this mapping will enable integration to send the user's response from  ChatBlend to the Google Sheet’s \*\*Name \*\*field in the leads. Similarly, in order for integration to work properly, you will require to map all other questions/ components of your Bot to Google Sheets fields.
...
Once you have established a connection between ChatBlend and **Google Sheets**, and the mapping is complete, you are ready to go for your first test lead. You can test your integration setup by creating a dummy lead on the bot. This lead should appear in your  ChatBlend dashboard as well as in the Google Sheets you have integrated.
...
<AccordionGroup>
  ...
  <Accordion title="How can I test whether my integration is working?">
    After setup, run a **dummy test lead** through the chatbot. If configured properly, the data should appear in your **Google Sheet** and also reflect in your \*\* ChatBlend dashboard.\*\*
  </Accordion>
...
- Wrong Mapping Set up - Your mapping with  ChatBlend Question and App parameter is either incorrect or you haven’t added any mandatory parameter.
- Free plan- You are not subscribed to the paid plan on  ChatBlend as integration works only in paid plan.