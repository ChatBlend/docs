---
title: "Integrate  ChatBlend with GoHigh Level"
description: "This is the guide for integrating  ChatBlend with GoHighLevel Integration"
---
...
- **Streamlined Lead Management** – Capture and sync  ChatBlend leads directly into GoHighLevel’s CRM for centralized tracking.
- **Automated Follow-Ups** – Trigger SMS, email, and voicemail campaigns based on chatbot interactions without manual intervention.
- **Unified Customer Journey** – Combine GoHighLevel funnels and  ChatBlend conversations for a seamless engagement flow.
- **Smart Workflow Automation** – Use  ChatBlend chat events to initiate GoHighLevel workflows and nurture sequences.

---

## **Steps for Integration:**

#### **Step 1. Navigating to GHL in  ChatBlend:**

1. Log into your  ChatBlend account, and go to your desired bot. Click on “**Third-party Apps Integrations**” option to go to all available 3rd Party Integrations page.
...
3. **Step 4: Set Up Mapping Parameters**
   ...
   Now that the connection between  ChatBlend and GoHighLevel (GHL) CRM is established, you need to map parameters (fields) of GHL CRM to the user responses from  ChatBlend's chatbot.
   1. You can use the **\+Add Parameters** button to add more fields.

      <Info>
        **Example Mapping Process**
        1. **Select a Question from  ChatBlend Chat Flow**
...
2. As a result, this mapping will enable the integration to **send the user's response from  ChatBlend to GoHighLevel's CRM fields**, storing it correctly in the contact records.
